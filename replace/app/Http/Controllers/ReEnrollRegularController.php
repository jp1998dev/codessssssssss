...
public function reEnrollIrregular(Request $request)
{
    // Validate the request
    $validated = $request->validate([
        'student_id' => 'required|exists:admissions,student_id',
        'course_mapping_id' => 'required|exists:program_course_mappings,id',
        'tuition_fee' => 'required|numeric',
        'misc_fees' => 'sometimes|array',
        'courses' => 'required|array',
        'courses.*.course_id' => 'required|exists:courses,id',
        'courses.*.override_prereq' => 'sometimes|boolean',
    ]);
    ...
    // Handle scholarship and discounts
    ...
    $totalAssessment = $tuitionFeeDiscount + $miscFeeTotal;
    ...
    // Assign student courses (for irregular students)
    $coursesData = [];
    ...
    return redirect()->back()->with('success', 'Irregular student re-enrolled successfully! Billing and course records created.');
}
...
public function calculateIrregularTuition(Request $request)
{
    ...
    // Check for 4th year special unit price
