...
class VPAdminSideBarController extends Controller
{
    public function studentSummary()
    {
        try {
            ...
            return view('vp_admin.summary.student-summary', compact(
                'schoolYears',
                'programs'
            ));
        } catch (\Exception $e) {
    ...
    public function shsSummary()
    {
        try {
            $schoolYears = SchoolYear::all();
            $strands = Strand::all();
            return view('vp_admin.summary.shs-summary', compact(
                'schoolYears',
                'strands'
            ));
        } catch (\Exception $e) {
    ...
    public function getShsSummary($schoolYearId)
    {
        try {
            ...
            foreach ($schoolYears as $activeSchoolYear) {
                ...
                $summaryTotals['esc_total'] += ShsEnrollment::where('school_year', $activeSchoolYear->name)
                    ->where('type_of_payee', 'With ESC')
                    ->count();
                $summaryTotals['voucher_total'] += ShsEnrollment::where('school_year', $activeSchoolYear->name)
                    ->where('type_of_payee', 'With Voucher')
                    ->count();
                $summaryTotals['novoucher_total'] += ShsEnrollment::where('school_year', $activeSchoolYear->name)
    ...
    public function getOldAccounts()
    {
        try {
            ...
            return view('vp_admin.old.old-accounts', compact('oldAccounts'));
        } catch (\Exception $e) {
            return view('vp_admin.old.old-accounts')->with('error', 'Something went wrong.');
        }
    }
    public function getBankDeposits(Request $request)
    {
        ...
        return view('vp_admin.bank.bank-deposit', compact('deposits', 'systemTotal', 'totalBanked'));
    }
    ...
    public function printCollection($id)
    {
        ...
        return view('vp_admin.collection-summary-print', compact(
            'collection',
            'collegePayments',
            'shsPayments',
            'otherPayments',
    ...
    public function oldTransactions()
    {
        try {
            ...
            // ]);
            return view(
                'vp_admin.transactions.old-transactions',
                [
                    'payments' => $payments
                ]
            );
    ...
    public function uniformTransactions()
    {
        try {
            ...
            // ]);
            return view(
                'vp_admin.transactions.uniform-transactions',
                [
                    'shspayments' => $shspayments,
                    'collegespayments' => $collegespayments
                ]
    ...
    public function otherTransactions()
    {
        try {
            ...
            // ]);
            return view(
                'vp_admin.transactions.other-transactions',
                [
                    'shspayments' => $shspayments,
                    'collegespayments' => $collegespayments
                ]
    ...
    public function shsTransactions()
    {
        try {
            ...
            return view('vp_admin.transactions.shs-transaction', ['payments' => $payments]);
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    }
    public function transactions()
    {
        ...
        return view('vp_admin.transactions.transactions', compact('payments', 'admissions'));
    }
    // Main dashboard
